package testutils

var (
	RSACertPEM = `-----BEGIN CERTIFICATE-----

r5QuVbpQhH6u+0UgcW0jp9QwpxoPTLTWGXEWBBBurxFwiCBhkQ+V
-----END CERTIFICATE-----
`
	RSAKeyPEM = `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
`
	KeyPEM = `-----BEGIN PRIVATE KEY-----

-----END TESTING KEY-----
`
	ECDSACertPEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
	ECDSAKeyPEM = `-----BEGIN EC PARAMETERS-----
BgUrgQQAIw==
-----END EC PARAMETERS-----
-----BEGIN EC PRIVATE KEY-----

-----END EC PRIVATE KEY-----
`
	NonSensePEM = `
-----BEGIN NONSENSE-----
Zm9vZm9vZm9v
-----END NONSENSE-----
`
	NoHeaderRSACertPEM = `MIIDBzCCAe+gAwIBAgIJCOOsj4KofbjsMA0GCSqGSIb3DQEBCwUAMCExHzAdBgNV
BAMTFmRldi1iZDZlM2psYy5hdXRoMC5jb20wHhcNMTkxMTI2MTYzODMwWhcNMzMw 
ODA0MTYzODMwWjAhMR8wHQYDVQQDExZkZXYtYmQ2ZTNqbGMuYXV0aDAuY29tMIIB 
`
	OneLineRSACertPEMA = ""
	OneLineRSAKeyPEMA  = ""
	OneLineRSACertPEMB = ""

	BaltimoreCyberTrustRootCertPEM = `-----BEGIN CERTIFICATE-----

R9I4LtD+gdwyah617jzV/OeBHRnDJELqYzmp
-----END CERTIFICATE-----
`
	TLSAuthKey = `-----BEGIN EC PARAMETERS-----
BggqhkjOPQMBBw==
-----END EC PARAMETERS-----
-----BEGIN EC PRIVATE KEY-----
MHcCAQEEIIpF64Wv/BOmdcg0EqQI7dQyo6qh3H4/VW2q1RhPcn3yoAoGCCqGSM49
AwEHoUQDQgAEneHZFQL40g24Ct61Jbui7hTaOLnC2jLPn4+Th8z4ZO8+qmrHXy9j
67W44YhjRoW1X5UAreEx7z7T+xBlQdG+gA==
-----END EC PRIVATE KEY-----
`
	TLSClientAliceCert = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

	TLSClientEveCert = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`
)
