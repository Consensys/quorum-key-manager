...
func (auth Authenticator) Authenticate(req *http.Request) (*entities.UserInfo, error) {
	...
	userInfo.Username, userInfo.Tenant = authenticator.ExtractUsernameAndTenant(clientCert.Subject.CommonName)
	userInfo.Permissions = authenticator.ExtractPermissionsArr(clientCert.Subject.OrganizationalUnit)
